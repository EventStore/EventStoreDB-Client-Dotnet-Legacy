...
namespace EventStore.ClientAPI.Internal {
	internal class ClusterDnsEndPointDiscoverer : IEndPointDiscoverer {
		...
		private async Task<NodeEndPoints?> DiscoverEndPoint(EndPoint failedEndPoint) {
			...
			for (int i = 0; i < gossipCandidates.Length; ++i) {
				var gossip = await TryGetGossipFrom(gossipCandidates[i]).ConfigureAwait(false);
		...
		private Task<ClusterMessages.ClusterInfoDto> TryGetGossipFrom(GossipSeed endPoint) {
			//_log.Debug("ClusterDnsEndPointDiscoverer: Trying to get gossip from [{0}].", endPoint);

			var completedSource = new TaskCompletionSource<ClusterMessages.ClusterInfoDto>();
